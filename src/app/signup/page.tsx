...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: { data: { full_name: fullName }, emailRedirectTo: `${window.location.origin}/auth/callback?redirect=/dashboard` },
    });
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ background: "#0a0b0f" }}>
      ...
      <div className="relative z-10 w-full max-w-[420px] px-5">
        <div className="text-center mb-8">
          ...
          <p className="text-sm" style={{ color: "rgba(255,255,255,.5)" }}>Start with 3 free chart scans daily</p>
        </div>
        {success ? (
          <div className="rounded-2xl p-6 text-center" style={{ background: "rgba(0,229,160,.06)", border: "1px solid rgba(0,229,160,.15)" }}>
            <div className="text-3xl mb-3">✉️</div>
            <h2 className="text-lg font-bold text-white mb-2">Check your email</h2>
            <p className="text-sm" style={{ color: "rgba(255,255,255,.5)" }}>We sent a confirmation link to <strong style={{ color: "#00e5a0" }}>{email}</strong>. Click it to activate.</p>
          </div>
        ) : (
